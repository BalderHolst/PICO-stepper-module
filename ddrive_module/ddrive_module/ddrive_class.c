...
static mp_obj_t DiffDrive_deinit(mp_obj_t self_in) {
    ...
    m_del(int, ddrive_instance, STEPPER_PINS);
    m_del(int, self->ddrive.rpins, STEPPER_PINS);
    m_del(float, self->ddrive.sequence.items, self->ddrive.sequence.length * STEPPER_PINS);
